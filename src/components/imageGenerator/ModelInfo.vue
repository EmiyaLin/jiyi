  ...
  <div>
    ...
    >
      <span class="demonstration" style="line-height: 30pt"
        >绘画风格{{ cardSelectTitle }}</span
      >
    </el-tooltip>
    <div class="card-container">
      <el-card
        v-for="(item, index) in cardList.slice(0, 2)"
        :key="index"
      ...
      >
        <div
          class="card-image"
          :style="{
            'background-image': `url(${serverAddr}/api/getModelImg?mid=${item.number})`,
          }"
        ></div>
        <div class="card-content" v-if="!item.selected"></div>
    ...
    <span v-if="cardList.length > 2">
      <el-collapse :gutter="16">
        <el-collapse-item title="更多绘画风格">
          <div class="card-container">
            <el-card
              v-for="(item, index) in cardList.slice(2, 99)"
              :key="index"
              :class="{ selected: item.selected }"
              @click.native="selectCard(index + 2)"
            >
              <div
                class="card-image"
                :style="{
    ...
    <el-drawer
      ...
      v-model="dialogModelSettingVisible"
      title="设置模型"
      size="600"
      :direction="createImageDialogType"
    >
      <!-- Drawer 内容 -->
      <div>
        <!-- 主要内容 -->
...
export default {
  ...
  methods: {
    ...
    selectCardAdvance() {
      this.$myFetch("/api/listModel", "POST", null).then((result) => {
